...
In the [previous tutorial](https://github.com/ebeneditos/telegram.bot/wiki/Tutorial-–-Your-first-R-Bot-in-3-steps) we saw how to build a Telegram Bot with R. However, it can now only answer to the `/start` command, so now we are going to add a couple of functionalities, including:
...
You can read more about customising filters in the next page: [Custom Filters](https://github.com/ebeneditos/telegram.bot/wiki/Custom-Filters).

### Attribution

This vignette is partially adapted from [`python-telegram-bot` Wiki](https://github.com/python-telegram-bot/python-telegram-bot/wiki/Extensions-–-Your-first-Bot).
