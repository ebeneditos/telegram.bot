...
Say that for the `kill` example we saw in the [previous page](https://github.com/ebeneditos/telegram.bot/wiki/Basic-Functionalities#stopping-the-bot), we would like to filter that command so to make it accessible only for a specific `<user-id>`. Thereby, you could add a filter:

```{r, eval = F}
filter_user <- function(message) message$from_user  == <user-id>
```
...
```{r, eval = F}
filter_user <- as.BaseFilter(function(message) message$from_user  == <user-id>)
```
